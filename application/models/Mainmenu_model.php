...
class Mainmenu_model extends CI_Model
{
    ...
    public function insert_main_menu($data, $sess)
    {
        ...
        $sql_check_max = "SELECT IFNULL(MAX(smm_order), 0) + 1 AS next_order_no FROM sys_main_menu;";
        ...
        if ($query_check_duplicate->num_rows() > 0) {
            ...
            $sql_insert = "INSERT INTO sys_main_menu (smm_name, smm_icon, smm_order, smm_status_flg, smm_created_date, smm_created_by)
    ...
    public function update_mmn($data, $sess)
    {
        ...
        $sql = "UPDATE sys_main_menu SET smm_order = $ordno  WHERE  smm_id = $id ";
        $query_update = $this->db->query($sql);

        $sql1 = "SELECT * FROM sys_main_menu WHERE smm_order >= $ordno  AND smm_id != $id ORDER BY smm_id ASC";
        ...
        foreach ($row1 as $items) {
            $gid = $items["smm_id"];
            $oldOrder = $items["smm_order"];
            $Neworder =  $Neworder + 1;

            if ($gid) {
                $sqlUpdate = "UPDATE sys_main_menu SET smm_order = $Neworder  WHERE smm_id = $gid";
            }
    ...
    public function edit_main_menu($data, $sess)
    {
        ...
        if (!$checkEdit) {
            $sql = "UPDATE sys_main_menu SET smm_name = '$mmname', smm_icon = '$mmicon', smm_order = '$ordno', smm_updated_date = NOW(), smm_updated_by = '$sess' WHERE smm_id = '$id'";
    ...
    public function orderNo($data)
    {
        ...
        $result = $this->db->query("SELECT smm_id,smm_order FROM sys_main_menu");
        ...
        foreach ($res as $value) {
            if ($value["smm_id"] != $id) {
                $i = $i == $ordno ? ++$i : $i;
                $result = $this->db->query("UPDATE sys_main_menu SET smm_order = '$i' WHERE smm_id = '{$value['smm_id']}'");
                $i++;
